(function() {
  ...
  function filterValidUnits(units) {
  ...
  function getChapterKey(unit) {
    if (unit.contextChain.length > 0) {
      return unit.contextChain[0];
    }
    return "__root__";
  }
  function createBatches(chapters) {
    const batches = [];
    let currentBatch = [];
  ...
  function extractRequestData(units) {
  ...
  class GlossaryManager {
     ...
     * @param domain - 领域
     */
    static async addTerm(term, translation, domain) {
      try {
  ...
  class TranslationQueue {
     ...
     * 开始翻译流程
     */
    async start(batches, progressCb, completeCb, errorCb) {
     ...
     * 滑动窗口并发处理
     */
    async processWithSlidingWindow() {
      ...
      GlossaryManager.getGlossaryDict().then((glossary) => {
        ...
        chrome.runtime.sendMessage(message, (response) => {
          ...
          if (!response || !response.success) {
            const errorMsg = (response == null ? void 0 : response.error) || "翻译请求失败";
          ...
          const translatedUnits = response.units;
     ...
     * 处理批次错误
     */
    handleBatchError(failedIds, error) {
  ...
  function renderTranslation(units, mode = "bilingual") {
    for (const unit of units) {
    ...
    try {
      switch (message.type) {
        case "START_TRANSLATE":
          ...
          return { success: true };
        case "TOGGLE_DISPLAY":
          currentMode = message.payload || "bilingual";
